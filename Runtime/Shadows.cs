...
public class Shadows
{
    ...
    private Matrix4x4[] otherShadowMatrices = new Matrix4x4[maxShadowedOtherLightCount];
    ...
    public Vector4 ReserveOtherShadows(Light light, int visibleLightIndex)
    {
        ...
        var data = new Vector4(light.shadowStrength, shadowedOtherLightCount, isPoint?1f:0f, maskChannel);
    ...
    private void RenderDirectionalShadows()
    {
        ...
        int split = CalculateSplitNumFromDesiredTileNum(tiles);
    ...
    private static int CalculateSplitNumFromDesiredTileNum(int tiles)
    {
        return tiles <= 1 ? 1 : tiles <= 4 ? 2 : 4; // tiles <= 16 ? 4 : 8;
    }
    ...
    private void SetKeywords(string[] keywords, int enabledIndex)
    {
        ...
        for (int i = 0; i < keywords.Length; i++) //directionalFilterKeywords.Length
        {
            if (i == enabledIndex)
            {
