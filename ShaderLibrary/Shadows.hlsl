...
#include "../ShaderLibrary/ThirdParty/ShadowSamplingTent.hlsl"
...
#define OTHERL_FILTER_SETUP SampleShadow_ComputeSamples_Tent_3x3
...
UNITY_DECLARE_SHADOWMAP(_DirectionalShadowAtlas);
UNITY_DECLARE_SHADOWMAP(_OtherShadowAtlas);
...
{
    return UNITY_SAMPLE_SHADOW(_DirectionalShadowAtlas, positionSTS);
}

float SampleOtherShadowAtlas(float3 positionSTS, float3 bounds)
{
    positionSTS.xy = clamp(positionSTS.xy, bounds.xy, bounds.xy+bounds.z);
    return UNITY_SAMPLE_SHADOW(_OtherShadowAtlas, positionSTS);
}
...
float GetCascadedShadow(DirectionalShadowData directional, ShadowData global, Surface surfaceWS)
{
    ...
    if (global.cascadeBlend < 1.0) //don't double the shadow sampling!!!!!! pls T_T
    {
