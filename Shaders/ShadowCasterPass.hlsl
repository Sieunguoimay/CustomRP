...
void ShadowCasterPassFragment(Varyings input)
{
    ...
    float4 base = GetBase(config); //baseMap * baseColor;

    #if defined(_SHADOWS_CLIP)
    clip(base.a - GetCutoff(config));//UNITY_ACCESS_INSTANCED_PROP(UnityPerMaterial, _Cutoff));
    #elif defined(_SHADOWS_DITHER)
    float dither = InterleavedGradientNoise(input.positionCS_SS.xy);
    clip(base.a - dither);
    #endif
}
#endif
