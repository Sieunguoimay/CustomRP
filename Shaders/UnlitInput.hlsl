...
{
    float4 rawMap = tex2D(_DistortionMap, c.baseUV);
    if (c.flipbookBlending)
    {
        rawMap = lerp(rawMap, tex2D(_DistortionMap, c.flipbookUVB.xy), c.flipbookUVB.z);
    }
    return UnpackNormalWithScale(rawMap, INPUT_PROP(_DistortionStrength)).xy;
}
...
float4 GetBase(InputConfig c)
{
    ...
    if (c.flipbookBlending)
    {
        map = lerp(map, tex2D(_BaseMap, c.flipbookUVB.xy), c.flipbookUVB.z);
    }
    if (c.nearFade)
    {
    ...
    if (c.softParticles)
    {
        ...
        map.rgba *= saturate(nearAttenuation);
    }
